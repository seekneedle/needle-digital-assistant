...
def to_prompt(role, context):
    ...
    tags = {
        "服务之星": "全程使用标准话术且耐心解答客户的问题，不遗漏，不忽略",
        "行程专家": "对旅游产品、旅游目的地很熟悉，且向用户介绍清晰，推荐精准",
        "销售达人": "需求洞察明确，擅长解决争议并引导客户报名，成交转化率高"
    }
    ...
    prompt = f'''
        ...
        tags，类型为 array[str]，按照标签和标签说明，给该客服人员（user）打若干标签，如果每一个标签都不符合，返回空列表。
        
        # 限制
        要求必须能力符合标签说明才返回对应标签，如果得分不高，不要返回相应标签。
    '''
    return prompt
...
async def trainer_score(request: TrainerScoreRequest):
    ...
    response = await llm.qwen_call(messages, 'json_object', '', 'score', 'qwen-max')
