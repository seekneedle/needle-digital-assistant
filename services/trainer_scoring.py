...
def to_prompt(role, context):
    ...
    evals = {
        '服务规范': '总分15分，打分标准：0-6分：遗漏关键服务话术/用语错误（如未说欢迎语直接询问需求）；7-12分：准确使用标准话术但缺乏温度；13-15分：灵活结合标准话术与个性化服务',
        '需求洞察': '总分20分，打分标准：0-8分：仅回应表面需求；9-16分：封闭式问题确认基础需求；17-20分：通过客户资料深度挖掘需求',
        '方案定制': '总分18分，打分标准：0-7分：推荐标准套餐；8-14分：组合调整服务模块；15-18分：场景重构设计专属行程',
        '信任建立': '总分18分，打分标准：0-7分：基础情绪识别（如"您似乎有些犹豫，需要我再说明吗？"）；8-14分：结合需求说明品牌优势（如："您重视深度体验，这正是我们“拒绝打卡式旅行”的理念"）；15-18分：故事化表达引发共鸣（如"南极之旅的真实案例+价值观植入"）',
        '异议转化': '总分15分，打分标准：0-6分：直接反驳客户（如"这个价格已经很便宜了"）；7-12分：提供附加补偿（如"今日签约免费升级头等舱休息室"）；13-15分：心理账户理论转化异议（如"高价对应30%专业服务与成本，是行程安全与舒适度的保障"）',
        '成交引导': '总分14分，打分标准：0-5分：被动等待成交（如"您考虑好了随时联系我"）；6-11分：限时促销策略（如"本周签约立减2000元"）；12-14分：预演产品使用场景（如"想象孩子触摸南极企鹅时的光芒，这是终身教育投资"）'
    }
